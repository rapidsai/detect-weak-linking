...
class ElfEntity:
    ...
    def __init__(self, file_path: str, no_ptx: bool) -> None:
        ...
        self.cuda_public_entry_symbols, self.cuda_private_entry_symbols = (
            get_cuda_entry_symbols(self.file, no_ptx)
        )
...
class ElfCache:
    ...
    def __init__(
        self, multiple_entries: bool, show_variables: bool, exclusions, no_ptx: bool
    ) -> None:
            ...
            if "RUNPATH" in line or "RPATH" in line:
                ...
                if entries == "$ORIGIN":
                    ...
                    entries = entries.split(
                        ":"
                    )  # Can have multiple values in the same entry
                    directories += entries

        for lib in libnames:
            lib_to_load = [
                os.path.join(dir, lib)
    ...
    def find_issues(self, issue_mode):
        ...
        if self.has_exclusions:
            json_entries = {
                ...
                if not has_match(v["symbol"], self.exclusions)
            }
...
def remove_baseline_entries(issues, baseline):
    ...
    def normalize_names(entry):
        normalized = {
            os.path.basename(k): v for k, v in entry.items() if not k == "symbol"
        }
        return normalized
...
def main():
    ...
    modes.add_argument(
        ...
        action="store_const",
        const="cuda",
        help="Use CUDA language kernel visibility requirements",
    )
    modes.add_argument(
        "--all-kernels",
        dest="mode",
        action="store_const",
        const="all_cuda_kernels",
        help="Consider all CUDA kernels not just weak/global ones",
    )
    modes.add_argument(
        "--all-weak",
        dest="mode",
        action="store_const",
        const="all",
        help="Consider all weak/global symbols not just CUDA kernels",
    )
    ...
    parser.add_argument(
        ...
        dest="global_vars",
        action="store_true",
        help="Show global unique variables",
    )
    parser.add_argument(
        "-m",
        "--multiple-instances",
        dest="multiple_entries",
        action="store_true",
        help="Only show symbols that are in multiple files",
    )
    parser.add_argument(
        "--no-ptx",
        dest="no_ptx",
        action="store_true",
        help="Don't looks for PTX kernel entries",
    )
    parser.add_argument(
        "-e",
        "--exclude",
        type=str,
        nargs="+",
        help="Exclude symbols that match this pattern ( applied on demangled names)",
    )
    parser.add_argument(
        "-b",
        "--baseline",
        type=argparse.FileType("r"),
        help="Show only results that are not in the baseline file",
    )
    parser.add_argument(
